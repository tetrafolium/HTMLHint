...
const defaultFormatter: FormatterCallback =
    function(formatter, HTMLHint, options) {
  const nocolor = !!options.nocolor

  formatter.on('start', () => {console.log('')})
  ...
  arrLogs.forEach((str) => {console.log(str)})
    ...
    formatter.on('end', (event) => {
      ...
      if (allHintCount > 0) {
        ...
        console.log(nocolor ? message : chalk.red(message), allFileCount,
                    allHintCount, allHintFileCount, time)
      }
      else {
        message = 'Scanned %d files, no errors found (%d ms).'
        console.log(nocolor ? message : chalk.green(message), allFileCount,
                    time)
      }
    })
}
