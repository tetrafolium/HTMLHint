...
console.log('    htmlhint --rulesdir ./rules/')
  ...
  program.version(pkg.version)
      ...
      .option('-r, --rules <ruleid, ruleid=value ...>',
              'set all of the rules available', map)
      .option('-R, --rulesdir <file|folder>',
              'load custom rules from file or folder')
      .option(`-f, --format <${arrSupportedFormatters.join('|')}>`,
              'output messages as custom format')
      .option('-i, --ignore <pattern, pattern ...>',
...
rulesdir?: string
    ignore?: string
      ...
      let arrAllMessages: Array < {file : string
      messages: Hint[]
    time: number
  }> = []
  let allFileCount = 0
    ...
    function loadCustomRules(rulesdir: string) {
      ...
      if (existsSync(rulesdir)) {
        if (statSync(rulesdir).isDirectory()) {
          ...
          arrFiles.forEach((file) => {loadRule(file)})
        } else {
          loadRule(rulesdir)
        }
      }
...
formatter: Formatter
  ...
  onFinised: (result: {
    ...
    arrTargetMessages: Array < {file : string
    messages: Hint[]
      time: number
    }>
  }) => void
        ...
        const arrTargetMessages: Array < {file : string
        messages: Hint[]
    time: number
  }> = []
  ...
  let ruleset = options.ruleset
    ...
    const hintQueue = asyncQueue<string>((filepath, next) => {
      ...
      else {const messages = hintFile(filepath, ruleset)
      hintNext(messages)
    }
  ...
  hintQueue.drain(() => {
    ...
    walkPath(
      ...
      function getGlobInfo(target: string): {base: string
      pattern: string
  ignore?: string
} {
    // fix windows sep
  ...
  function getConfig(configPath: string|undefined, base: string,
                     formatter: Formatter) {
  ...
  // walk path
  function walkPath(globInfo: {base: string; pattern : string; ignore?: string},
    ...
    const arrIgnores = [ '**/node_modules/**' ]
        ...
        if (ignore) {
      ignore.split(',').forEach((pattern) => {arrIgnores.push(pattern)})
    }

    const walk: IGlob = glob(pattern, {
      cwd : base,
      dot : false,
      ignore : arrIgnores,
      nodir : true,
  ...
  // hint stdin
  function hintStdin(ruleset: Ruleset|undefined,
                     callback: (messages: Hint[]) => void) {
    process.stdin.setEncoding('utf8')

    const buffers: string[] = []

    process.stdin.on('data', (text) => {buffers.push(text)})
  ...
  // hint url
  function hintUrl(url: string, ruleset: Ruleset|undefined,
                   callback: (messages: Hint[]) => void) {
