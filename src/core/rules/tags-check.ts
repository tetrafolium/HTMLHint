            ...
            parser.addListener('tagstart', (event) => {
              ...
              if (tagsTypings[tagName]) {
                const currentTagType = tagsTypings[tagName]
                  ...
                  reporter.warn(`The <${tagName}> tag must be selfclosing.`,
                                event.line, event.col, this, event.raw)
                }
                else if (currentTagType.selfclosing === false && event.close) {
                  reporter.warn(`The <${tagName}> tag must not be selfclosing.`,
                                event.line, event.col, this, event.raw)
                }
                ...
                if (Array.isArray(currentTagType.attrsRequired)) {
                  ...
                  attrsRequired.forEach((id) => {
                      ...
                      if (attrs.some((attr) => attr.name === realID)) {
                        attrs.forEach((attr) => {
                          if (attr.name === realID &&
                            ...
                            reporter.error(
                                `The <${tagName}> tag must have attr '${
                                    realID}' with one value of '${
                                    values.join("' or '")}'.`,
                                event.line, col, this, event.raw)
                          }
                        })
                      }
                      else {
                    ...
                    } else if (!attrs.some((attr) => id.split('|').indexOf(
                                                         attr.name) !== -1)) {
                      reporter.error(
                      ...
                      if (attrs.some((attr) => attr.name === realID)) {
                        attrs.forEach((attr) => {
                          if (attr.name === realID &&
                            ...
                            reporter.error(
                                `The <${
                                    tagName}> tag must have optional attr '${
                                    realID}' with one value of '${
                                    values.join("' or '")}'.`,
                                event.line, col, this, event.raw)
                          }
                ...
                if (Array.isArray(currentTagType.redundantAttrs)) {
                  ...
                  redundantAttrs.forEach((attrName) => {
                    if (attrs.some((attr) => attr.name === attrName)) {
                      reporter.error(
                          `The attr '${attrName}' is redundant for <${
                              tagName}> and should be ommited.`,
                          event.line, col, this, event.raw)
                    }
                  })
                }
