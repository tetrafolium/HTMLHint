...
export default {
  ...
  init(parser, reporter) {
    const labelTags: Array < {event : Block
    col: number
      forValue?: string
    }> = []
      ...
      parser.addListener('tagstart', (event) => {
        ...
        if (tagName === 'label') {
          if ('for' in mapAttrs && mapAttrs['for'] !== '') {
            labelTags.push(
                {event : event, col : col, forValue : mapAttrs['for']})
          }
        }
      })
      ...
      parser.addListener('end',
                         () => {inputTags.forEach((inputTag) => {
                           if (!hasMatchingLabelTag(inputTag)) {
                             reporter.warn('No matching [ label ] tag found.',
                                           inputTag.event.line, inputTag.col,
                                           this, inputTag.event.raw)
                           }
                         })})
