...
export type Listener = (event: Block) => void
  ...
  public lastEvent: Partial<Block>|null
      ...
      private _listeners: {[type: string]: Listener[]} private _mapCdataTags:
          {[tagName: string]: boolean} private _arrBlocks: Array<Partial<Block>>

      public constructor() {
    this._listeners = {} this._mapCdataTags =
        this.makeMap('script,style') this._arrBlocks = [] this.lastEvent = null
  }

  public makeMap(str: string): {[key: string]: boolean} {
    const obj: {[key: string]: boolean} = {} const items = str.split(',')
  ...
  public parse(html: string): void {
    ...
    let lastIndex = 0
    let tagName: string
    let arrAttrs: Attr[] let tagCDATA: string|null = null
    let attrsCDATA: Attr[]|undefined
    let arrCDATA: string[] = [] let lastCDATAIndex = 0
    let text: string
    let lastLineIndex = 0
    let line = 1
    ...
    // Memory block
    const saveBlock =
        (type: string, raw: string, pos: number, data?: Partial<Block>) => {
          ...
          data.line = line
          data.col = col
          arrBlocks.push(data) this.fire(type, data)
    ...
    while ((match = regTag.exec(html))) {
      ...
      if (tagCDATA) {
        ...
        if ((tagName = match[4])) {
          // Label start
          arrAttrs = [] const attrs = match[5] let attrMatch
          let attrMatchCount = 0

          while ((attrMatch = regAttr.exec(attrs))) {
            const name = attrMatch[1] const quote =
                attrMatch[2] ? attrMatch[2] : attrMatch[4] ? attrMatch[4] : ''
  ...
  public addListener(types: string, listener: Listener): void {
    ...
    for (let i = 0, l = arrTypes.length; i < l; i++) {
      type = arrTypes[i] if (_listeners[type] === undefined) {
        _listeners[type] = []
      }
  ...
  public fire(type: string, data?: Partial<Block>): void {
    ...
    let listeners: Listener[] = [] const listenersType =
        this._listeners[type] const listenersAll =
            this._listeners['all']
  ...
  public removeListener(type: string, listener: Listener): void {
    const listenersType: Listener[]|undefined =
        this._listeners[type] if (listenersType !== undefined) {
  ...
  public fixPos(event: Block, index: number): {line: number
    col: number
  } {
    ...
    public getMapAttrs(arrAttrs: Attr[]): {[name: string]: string} {
      const mapAttrs: {[name: string]: string} = {} let attr: Attr
