...
expect(messages[1].line).to.be(2)
  ...
  ruleOptions[ruldId] = [ 'testBox', 'tttAAA' ] const messages =
      HTMLHint.verify(code, ruleOptions)
    expect(messages.length).to.be(0)
  })

  it('Set to array list with RegExp should not result in an error', () => {
    const code = '<p testBox="abc" bind:tapTop="ccc">'
  ruleOptions[ruldId] = [ 'testBox', /bind:.*/] const messages =
      HTMLHint.verify(code, ruleOptions)
    expect(messages.length).to.be(0)
  })
  ...
  ruleOptions[ruldId] = [ 'testBox', '/\\[.*\\]/' ] const messages =
      HTMLHint.verify(code, ruleOptions)
    expect(messages.length).to.be(0)
  })
})
