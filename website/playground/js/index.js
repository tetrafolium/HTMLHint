...
var settings = {editorTheme : 'merbivore'};

var editor, arrHints = [];

var jHintState = $('#hint-state'), jButtonArea = $('#button-area'),
    jShowLast = $('#show-last'), jShowNext = $('#show-next'),
...
function saveRules() { $.cookie('htmlhintRules', ruleSets, {expires : 365}); }
...
function showLastHint() {
  if (arrHints.length > 0) {
    var cursor = editor.selection.getCursor(), curRow = cursor.row,
...
function initOptions() {
  ...
  $('input[type=checkbox]').change(function() {
    var jThis = $(this), id = jThis.attr('id'),
  ...
  $('select').change(function() {
    var jThis = $(this), id = jThis.attr('id').replace('_value', '');
    ruleSets[id] = jThis.val();
    saveRules();
    updateHTMLHint();
  });
  for (var id in ruleSets) {
    var ruleValue = ruleSets[id], jValue = $('#' + id + '_value');
